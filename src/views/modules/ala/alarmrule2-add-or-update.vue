    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.logId) {
            this.$http({
              url: this.$http.adornUrl(`/ala/alarmrule2/info/${this.dataForm.logId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.terminalId = data.alarmRule2.terminalId
                this.dataForm.sensorId = data.alarmRule2.sensorId
                this.dataForm.alarmLowLimit = data.alarmRule2.alarmLowLimit
                this.dataForm.alarmHighLimit = data.alarmRule2.alarmHighLimit
                this.dataForm.logUpdateTime = data.alarmRule2.logUpdateTime
                this.dataForm.logDescribe = data.alarmRule2.logDescribe
                this.dataForm.sensorSaveTable = data.alarmRule2.sensorSaveTable
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/ala/alarmrule2/${!this.dataForm.logId ? 'save' : 'update'}`),
              method: 'post',
