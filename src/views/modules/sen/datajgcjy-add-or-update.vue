    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sen/datajgcjy/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.sensorId = data.dataJgcjy.sensorId
                this.dataForm.sensorData = data.dataJgcjy.sensorData
                this.dataForm.date = data.dataJgcjy.date
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sen/datajgcjy/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
