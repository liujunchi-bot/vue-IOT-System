    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sen/datalxwyj/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.sensorId = data.dataLxwyj.sensorId
                this.dataForm.sensorData = data.dataLxwyj.sensorData
                this.dataForm.date = data.dataLxwyj.date
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sen/datalxwyj/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
