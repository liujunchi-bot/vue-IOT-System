    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sen/datadlcxy/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.sensorId = data.dataDlcxy.sensorId
                this.dataForm.sensorData = data.dataDlcxy.sensorData
                this.dataForm.date = data.dataDlcxy.date
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sen/datadlcxy/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
