    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sen/datajlszy/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.sensorId = data.dataJlszy.sensorId
                this.dataForm.sensorData = data.dataJlszy.sensorData
                this.dataForm.date = data.dataJlszy.date
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sen/datajlszy/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
