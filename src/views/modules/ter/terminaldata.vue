    ...
    >
      ...
      <el-form-item>
        ...
        <!-- <el-button
          round
          style="font-size: 18px"
          v-if="isAuth('ter:terminaldata:save')"
          type="primary"
        ...
        <el-button
          round
          style="font-size: 18px"
          v-if="isAuth('ter:terminaldata:delete')"
          type="danger"
          @click="deleteHandle()"
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            type="text"
            size="small"
            @click="deleteHandle(scope.row.dataId)"
            >删除</el-button
          >
        </template>
      </el-table-column> -->
...
<script>
import AddOrUpdate from './terminaldata-add-or-update'
export default {
  data () {
    return {
      tmpList: [],
  ...
  methods: {
    ...
    getDataList () {
      this.dataListLoading = true
      this.$http({
        url: this.$http.adornUrl('/ter/terminaldata/list'),
        method: 'get',
    ...
    deleteHandle (id) {
      ...
      this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        this.$http({
          url: this.$http.adornUrl('/ter/terminaldata/delete'),
          method: 'post',
